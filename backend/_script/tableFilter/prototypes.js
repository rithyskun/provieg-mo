...
(function(){
    function add(name,method){
        if(!String.prototype[name]){String.prototype[name]=method}
    };
    add("trim",function(){return this.replace(/^\s+|\s+$/g,"")});
    add("repeat",function(len){return new Array(len+1).join(this)});
    add("rJustify",function(len,chr){
        chr=(chr.toString().length)?chr.toString():" ";
        return chr.repeat(len-this.length||0)+this
    });
    add("lJustify",function(len,chr){
        chr=(chr.toString().length)?chr.toString():" ";
        return this+chr.repeat(len-this.length||0)
    })
})();
    
(function(){
    function add(name,method){
        if(!Array.prototype[name]){Array.prototype[name]=method}
    };

    add("forEach",function(handler,scope){scope=scope||window;for(var i=0;i<this.length;i++)handler.call(scope,this[i],i,this)});
    add("every",function(handler,scope){scope=scope||window;for(var i=0;i<this.length;i++)if(!handler.call(scope,this[i],i,this))return false;return true});
    add("some",function(handler,scope){scope=scope||window;for(var i=0;i<this.length;i++)if(handler.call(scope,this[i],i,this))return true;return false});
    add("map",function(handler,scope){scope=scope||window;var r=[];for(var i=0;i<this.length;i++)r[r.length]=handler.call(scope,this[i],i,this);return r});
    add("filter",function(handler,scope,returnFails){if(typeof scope=="undefined"){scope=window;returnFails=false}else if(typeof scope=="boolean"){scope=window;returnFails=scope}else{returnFails=(typeof returnFails=="boolean")?returnFails:false}var r=[];var t=[];var f=[];for(var i=0;i<this.length;i++)if(handler.call(scope,this[i],i,this)){t[t.length]=this[i]}else if(returnFails){f[f.length]=this[i]}if(returnFails){r.push(f);r.push(t)};return(returnFails)?r:t});
    add("indexOf",function(subject,offset){for(var i=offset||0;i<this.length;i++)if(this[i]===subject)return i;return-1});
    add("unique",function(){return this.filter(function(element,index,array){return array.indexOf(element)>=index})})
})();
