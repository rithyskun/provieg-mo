...
/**
 * Classe de gestion des demandes dans le reporting
 * @version 20070622
 * @author Antoine Marcadet & Stéphanie Léang
 */
class modelReportingRequest {
    ...
    public function insert($request_uri, $commentaire) {
        try {
        	...
        	$sql = "INSERT INTO report_demande (id_report, date_creation, id_createur, date_modification, id_modificateur, etat_doc, referer, commentaire, user_agent, id_priorite)
        	...
        	// INSERTION DEMANDE EN MODE ATTENTE
        	$id_report = $db->lastInsertId();
        	$sql = "INSERT INTO report_demande_etat (id_demande_etat, date_creation, id_createur, date_modification, id_modificateur, etat_doc, id_report, id_etat, commentaire)
            ...
            throw new Exception('Demande non insérée');
        }
    }
    
    
    public function update($id_demande, $com, $id_etat){
        try {
            $db = Annuaire::lookup(KEY_DATABASE);
            $db->autoCommit(false);
            $db->beginTransaction();
        	$sql =	"INSERT INTO report_demande_etat ( id_demande_etat , date_creation , id_createur,
					date_modification , id_modificateur , etat_doc , id_report , id_etat , commentaire )
					VALUES ('', '".time()."', '{$_SESSION['id_user']}', '".time()."', '{$_SESSION['id_user']}', 1, '$id_demande', '$id_etat', '$com');";
			$db->executeQuery($sql);
			
			$sql = "UPDATE report_demande
					SET date_modification = " . time() . ",
						id_modificateur = " . $_SESSION['id_user'] . "
					WHERE id_report = $id_demande";
			$db->executeQuery($sql);
			$db->commitTransaction();
		}
        catch(SQLException $e) {
    ...
    public function getList($search = null){
        try{
            ...
            $sql = "SELECT rp.*, au1.login AS login_createur, au2.login AS login_modificateur, der.lib_etat_report AS lib_etat, dpr.id_priorite, dpr.lib_priorite
                    ...
                    LEFT JOIN report_demande_etat AS rpe ON (rpe.id_report = rp.id_report AND rpe.id_demande_etat = (SELECT MAX(id_demande_etat) 
                                                                                                                     FROM report_demande_etat 
    ...
    public function getDemande($id_demande){
        try{
            ...
            $sql = "SELECT rp.*, au1.login AS login_createur, au2.login AS login_modificateur, dpr.lib_priorite
					FROM report_demande AS rp, _adm_user AS au1, _adm_user AS au2, zz_data_priorite_report AS dpr
                    WHERE au1.id_user = rp.id_createur
                    AND au2.id_user = rp.id_modificateur
                    AND rp.id_report = $id_demande
    ...
    public function getListStatus($id_demande){
        try{
            $db = Annuaire::lookup(KEY_DATABASE);
            return  $db->executeQuery("SELECT rp.*, au1.login AS login_createur, au2.login AS login_modificateur, etat.lib_etat_report
                        					FROM report_demande_etat AS rp, _adm_user AS au1, _adm_user AS au2, zz_data_etat_report AS etat
                        					WHERE (au1.id_user = rp.id_createur AND rp.id_report=$id_demande)
                        					AND au2.id_user = rp.id_modificateur
